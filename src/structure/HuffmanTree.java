...
public class HuffmanTree {
	 ...
	 * représentée en interne par un TreeSet (ARN) dans la clé est comparable ou on
	 * définie son ordre (Ce qu'on a fait pour Node), la valeur c'est la clé elle
	 * même identité (Un couple) (key=Valeur, value=Valeur)
	 */
	private final TreeSet<Node> gdbh = new TreeSet<>();

	/**
	 * contient tous les cars qui sont dans l'arbre (recherche en O(1) Table de
	 * Hash) Le plus intéressant on a une référence à la feuille de chaque caractère
	 */
	...
	/**
	 * Cette méthode permet d'ajouter ou mettre à jour une occurence d'un caractère
	 * dans l'AHD toute on conservant sa validité
	 * 
	...
	public void modification(String c) {
		...
		if (root == carSpecial) {
			/*
			 * L'arbre devient un AHD avec un noeud interne (pds 1) avec enfant gauche # et
			 * enfant droit [(C) (pds 1)]
			 */
			...
			// sinon on le calcule une fois dans ABR_NYT_CHAR(Q) && Q.getParent() ==
			// finBloc(Q)
			...
			if (feuille_c == null) {
				...
				/*
				 * Remplacer noeud(#) par noeud interne (pds 1) avec enfant gauche # et enfant
				 * droit s (pds 1)
				 */
...
//					System.out.println("Pas normal b ne peut pas etre NYT : " + res);
	...
	public Node estIncrementable(List<Node> path, SortedSet<Node> gdbh_q) {
		...
		for (Node sommet : path) {
			if (sommet != root) {
				...
				// mais on cherche le succ de 2 dans tout le gdbh y'a une occ de 2 donc n'est
				// pas inc
	...
	/**
	 * Avant O(m) Maintenant => Complexité O(log(m)) avec m taille de gdbh_q (notre
	 * liste de noeuds de parcours) Cette Méthode renseigne le suivant d'un noeud Q
	 * dans le parcours Gdbh si Q = X1 alors son successeur sera X2
	 * 
	 * On utilise NavigableSet<E> qui est une interface introduite dans Java 6 qui
	 * étend SortedSet<E> et fournit des méthodes supplémentaires pour naviguer dans
	 * un ensemble trié (set ordonné).
	 */
	public Node successeurNoeudGDBH(Node Q, SortedSet<Node> gdbh_q) {
		if (Q == null || Q == root)
			return Q;

		return ((NavigableSet<Node>) gdbh_q).tailSet(Q, false) // false => exclure Q lui-même
				.stream().findFirst().orElse(Q);
	}
...
//		}
	...
	/**
	 * Invariants: -1 La somme des occurences de left et right est égale à celle de
	 * this -2 Par construction, Tout noeud interne possède forcément exactement 2
	 * (fils), c'est à dire on peut pas avoir this avec avec left ou right qui est
	 * null
	 */
	public class Node implements Comparable<Node> {
		...
		/**
		 * La comparaision qui va nous permettre de mentenir une liste trié qui
		 * représente le parcour GDBH
		 * 
		...
		public int compareTo(Node o) {
			/*
			 * 
			 * Si la profondeur du noeud qu'on veut insérer est plus grande que celle du
			 * noeud courant donc il se code avec plus de car (il apprait moins) on le
			 * considère plus petit en terme d'occurence plus grand en codage
			 */
		...
		/**
		 * Attention! ne met pas à jour les occurence, profondeur et codes. Utilisez
		 * mej...() après parceque l'invariant 2 n'est pas forcément préserver après
		 * cette apelle
		 * 
		 * @param right
		 */
		...
		/**
		 * Met à jour la profondeur de chaque nœud de l’arbre binaire à partir du nœud
		 * courant.
		 * 
		 * La profondeur d’un nœud est définie comme sa distance par rapport à la racine
		 * (nœud de départ). Utilise un parcours en profondeur (DFS) itératif à l’aide
		 * d’une pile explicite, ce qui évite les risques de StackOverflowError qu'on
		 * aurait avec une récursion profonde.
		 * 
		 * Les feuilles (instances de {@code Leaf}) ne sont pas explorées, car elles
		 * n’ont pas d’enfants.
		 */
		public void majProfondeur() {
			...
			while (!stack.isEmpty()) {
				...
				if (!(current instanceof Leaf)) { // si ce n'est pas une feuille alors forcément on a deux fils (INVARIANT)
		...
		/**
		 * Attribue un code binaire à chaque nœud de l’arbre binaire, en partant du nœud
		 * courant.
		 * 
		 * Le code est construit en ajoutant "0" pour un enfant gauche, et "1" pour un
		 * enfant droit. Cette méthode est typiquement utilisée pour générer des codes
		 * de Huffman.
		 * 
		 * Utilise un parcours en profondeur (DFS) itératif avec une pile explicite, ce
		 * qui permet de gérer de grands arbres sans risque de StackOverflowError.
		 * 
		 * Les feuilles ne sont pas explorées (elles conservent leur code assigné depuis
		 * leur parent).
		 */
		public void majCode() {
			//removeFromGDBH(this);
		...
		protected String toStr() {
			return "occurence: " + this.occurence + " profondeur: " + this.profondeur + " code: " + this.code + "\n";
		}
